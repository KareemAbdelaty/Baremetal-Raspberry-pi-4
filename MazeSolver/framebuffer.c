...
//                  color1:              Color of the walls
//                  color2:              color of the empty paths
//					color3:				 color of the player
//
//  Returns:        void
//
//  Description:    This function draws a maze pattern on the display
//                  with the prescribed numbers of rows and columns and the
//                  specified square size.
//
////////////////////////////////////////////////////////////////////////////////

void drawMaze(int maze[12][16], int squareSize,
		       unsigned int color1, unsigned int color2,unsigned int color3,int mode)
{
    int i, j;
//if mode is 0 dont draw the player
	if(mode ==0){
		for (i = 0; i <12; i++) {
			for(j=0;j<16;j++){
				if(maze[i][j] == 0){
...
//if mode is 1 draw the player in red
	else if(mode ==1){
		for (i = 0; i <12; i++) {
			for(j=0;j<16;j++){
				if(maze[i][j] == 0){
					...
					drawSquare(i * squareSize, j * squareSize, squareSize, color3);
				}
				else if(maze[i][j] == 3){
					drawSquare(i * squareSize, j * squareSize, squareSize, color2);
				}
...
//if mode is 2 draw the player in green
	else if(mode ==2){
		for (i = 0; i <12; i++) {
			for(j=0;j<16;j++){
				if(maze[i][j] == 0){
...
//
//  Arguments:      a maze to be drawn and mode of drawing.
//					mode 0 doesnt draw the player
//					mode 1 draws the player in red
//					mode 2 draws the player in green
//  Returns:        void
//
//  Description:    This function displays a Maze, where each
//                  square is 64 x 64 pixels in size. Since the screen
//                  resolution is set to 1024 x 768, the Maze has 12 rows and 16 columns
//
////////////////////////////////////////////////////////////////////////////////
...
void displayFrameBuffer(int m[12][16],int mode)
{
    ...
    //Maze has 12 rows and 16 columns
    numberOfRows = 12;
    numberOfColumns = 16;
 
    // Draw Maze
	if(mode == 0){
		drawMaze(m, squareSize, Black, White,red,mode);		
	}	
	else if(mode ==1){
		drawMaze(m, squareSize, Black, White,red,mode);
	}
	else if(mode ==2){
		drawMaze(m, squareSize, Black, White,Green,mode);
	}
}
